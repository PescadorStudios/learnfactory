...
import type { BriefingCard, LessonStep, NodeResult, NodeType, Sintesis, SocraticEvaluation } from "@/lib/types";
...
export default function MicroLesson({ topic, nodeId, nodeTitle, nodeType, sintesis, conceptIds, onComplete, onExit }: Props) {
  ...
  const [phase, setPhase] = useState<"briefing" | "steps">("briefing");
  ...
  useEffect(() => {
    ...
    async function loadLesson() {
      // Intentar cache primero (los formatos viejos —arrays o sin briefing— se regeneran)
      const cacheKey = `learnfactory_lesson_${topic}_${nodeId}`;
      const cached = localStorage.getItem(cacheKey);
      if (cached) {
        try {
          const parsed = JSON.parse(cached);
          if (!ignore && Array.isArray(parsed?.briefing) && Array.isArray(parsed?.steps) && parsed.steps.length > 0) {
            setBriefingCards(parsed.briefing);
            setLessonSteps(parsed.steps);
            setPhase(parsed.briefing.length > 0 ? "briefing" : "steps");
            setLoading(false);
            return;
          }
        } catch {}
      ...
      setLoading(true);
      const data = await generateMicroLesson(
        topic, nodeTitle, nodeType, sintesis, conceptIds,
        getStudiedConceptIds(topic, conceptIds)
      );
      if (!ignore) {
        setBriefingCards(data.briefing || []);
        setLessonSteps(data.steps);
        setPhase((data.briefing || []).length > 0 ? "briefing" : "steps");
        setLoading(false);
        // Cachear para evitar doble llamada
  ...
  if (loading || lessonSteps.length === 0) {
    return (
      <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center text-white">
        ...
        <p className="text-zinc-400">Generando microlección a partir de tus fuentes...</p>
      </div>
    );
  }

  // Fase 1: briefing estilo "stories" antes de la lección
  if (phase === "briefing") {
    return (
      <Briefing
        cards={briefingCards}
        onFinish={(predictCorrect) => {
          if (predictCorrect) {
            xpEvents.current.push({ action: "prediccion_correcta", xp: XP.predictCorrect });
          }
          setPhase("steps");
        }}
        onExit={onExit}
